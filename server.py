...
def recuperer_pseudos_crunchyroll():
    ...
    crunchy_usernames = get_user_in_BDD()
    usernames_traiter = []  # Liste pour suivre les pseudos déjà traités

    for crunchy_username in crunchy_usernames:
        if crunchy_username[0] not in usernames_traiter:
            req_breach(crunchy_username[0])
            usernames_traiter.append(crunchy_username[0])
            print(f"Breach req for: {crunchy_username[0]}")
